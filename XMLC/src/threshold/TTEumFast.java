...
public class TTEumFast extends ThresholdTuning {
	...
	public double[] validate(AVTable data, AbstractLearner learner) {
		...
		for (int j = 0; j < data.n; j++) {
			...
			//HashSet<Integer> trueLabels = new HashSet<Integer>(Ints.asList(data.y[j]));
			...
			for (EstimatePair pred : sPE) {
				...
				if (posteriors[label] == null) {
					posteriors[label] = new ArrayList<ComparableTriplet>();
				}
				posteriors[label].add(new ComparableTriplet(pred.getP(), j, y));
			}
		...
		for (int i = 0; i < posteriors.length; i++) {
			...
			// tune the threshold
			// every instance is predicted as positive first with a threshold =
			// 0.5
			double maxthreshold = 1.0;
			int tp = 0;
			int predictedPositives = 0;
			double Fmeasure = 0; // ((2.0*tp)) / ((double) ( numPositives[i] +
									// predictedPositives ));
			...
			// logger.info("Label: " + i + " threshold: " + thresholds[i] + " F:
			// " + maxFmeasure);
		}

		// for( int i=0; i < this.m; i++ )
		// logger.info( "Class: " + i + " Th: " + String.format("%.4f",
		// this.thresholds[i]) );
